...
export function ISSBScoringTab({ customerId, year }: ISSBScoringTabProps) {
  const [rows, setRows] = useState<ISSBScore[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    setMessage(null);
    try {
      const data = await apiRequest<{ rows: ISSBScore[] }>(
        `/api/dma/issb-assessment?customerId=${encodeURIComponent(customerId)}&year=${year}`,
      );
      setRows(data.rows || []);
    } catch (err: any) {
  ...
  const saveAll = useCallback(async () => {
    ...
    try {
      ...
      await apiRequest('/api/dma/issb-assessment/scores', {
        method: 'POST',
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div>
          ...
          <p className="text-xs text-blue-700 mt-1">
            1-5 arası puan girerek ISSB (IFRS S1/S2) materiyallik kriterlerini belirleyin. 4 ve üzeri puan "Önemli" olarak işaretler.
          </p>
        </div>
      ...
      {rows.length === 0 && (
        <div className="text-center py-8 text-slate-500">
          <p className="text-sm">ISSB konuları yüklenemedi. Lütfen sayfayı yenileyin.</p>
        </div>
      )}
    </div>
  );
