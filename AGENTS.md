...
> **In progress: MongoDB → relational (SQL) migration.** A Prisma-based PostgreSQL/MySQL backend is being introduced behind a `DB_DRIVER` flag (defaults to `mongo`, so current behavior is unchanged). When `DB_DRIVER=sql`, the generic `/api/db/:resource` API and settings/answer upsert route through a repository seam in `server/data/**` instead of Mongoose. RAG vectors move to `pgvector` (`VECTOR_BACKEND`). See [`docs/migration/mongo-to-sql-migration-plan.md`](docs/migration/mongo-to-sql-migration-plan.md) and [`docs/migration/mongo-to-sql-implementation-status.md`](docs/migration/mongo-to-sql-implementation-status.md). Do not assume Mongo-only; check `DB_DRIVER` when touching the data layer.
...
```bash
npm run migrate:import path/to/firestore-export.json   # Import legacy Firestore data
npm run migrate:validate                               # Validate MongoDB migration integrity
npm run migrate:platform-roles                         # Normalize legacy platform roles
npm run migrate:soru-cogaltma                          # Backfill Turkish question multiplier
npm run seed:dump                                      # mongodump to seed/mongo-dump
npm run seed:restore                                   # mongorestore from seed/mongo-dump
```

### Relational (SQL) Migration — in progress
```bash
...
npm run audit:mongo              # Phase 0 read-only data-quality audit
npm run migrate:mongo-to-sql     # ETL: Mongo -> SQL (canonical-id remap); add -- --dry-run
npm run migrate:validate-sql     # cross-DB row-count parity check
npm run migrate:embeddings-pgvector   # copy KB vectors Qdrant -> pgvector
# Then run app with: DB_DRIVER=sql VECTOR_BACKEND=pgvector (DATABASE_URL/VECTOR_DATABASE_URL set)
```

### Docker & Local Stack
```bash
docker compose up --build   # Full stack: app + MongoDB (27018) + Postgres/pgvector (5432) + MinIO (9000/9001)
...
- **Database**: MongoDB 7 + Mongoose 9 ODM (default). Pluggable relational core via Prisma 6 (PostgreSQL/MySQL) behind `DB_DRIVER=sql` — migration in progress.
...
│    • Generic Mongo REST API (/api/db/:resource/:id)      │
...
│   │   │   └── [more...]
│   ├── services/
│   │   ├── db.ts                  # Firestore→MongoDB compatibility layer
│   │   │   # Maps Firestore-style collection(path).where(...).getDocs()
│   │   │   # to /api/db/* REST endpoints; allows gradual migration
...
│   └── hooks/                     # React hooks
│
├── server.ts                      # Express entry point (56KB; handles auth + setup)
├── server/
│   ├── db/mongo.ts                # MongoDB connection + health check
│   ├── models/index.ts            # Mongoose schemas (Customer, Project, Question, Answer, etc)
│   ├── routes/
│   │   ├── mongoApi.ts            # Generic CRUD endpoints /api/db/:resource
...
│   │   ├── [8 more domain utilities]
│   │   └── platformRoles.ts       # Role normalization (synced with client)
│   ├── auth/password.ts           # crypto.scrypt hashing
│   ├── migrations/                # MongoDB migration helpers
│   │   ├── migrateDomainCategories.ts
│   │   ├── migrateCustomerSectorClassification.ts
│   │   ├── migrateLegacyPlatformRoles.ts
│   │   └── backfillSoruCogaltma.ts
│   └── migration/
│       └── firestoreExport.ts     # Firestore JSON import logic
│
├── scripts/                       # One-off utility scripts
│   ├── import-firestore-export.ts # CLI for Firestore import
│   ├── validate-mongo-migration.ts
│   └── [4 more]
│
...
├── docker-compose.yml             # App + MongoDB + MinIO
...
Core entities organized by responsibility:
...
All schemas include:
- `legacyFirebaseId` (tracks Firestore migration)
- `createdBy`, `ownerId` (ownership/audit)
- Mongoose timestamps (`createdAt`, `updatedAt`)
- Virtual `id` getter (maps `_id.toString()` for frontend compatibility)
...
### Platform Roles
Defined in both `lib/platformRoles.ts` and `server/models/index.ts` (must stay in sync):
...
```
PORT=3010
MONGODB_URI=mongodb://localhost:27018/governance
...
# Expected: { "status": "ok", "mongoReady": true, "env": "development" }
```
...
- Migration validation passes (if importing Firestore data)
...
- `lib/platformRoles.ts` (shared) and `server/models/index.ts` (Mongoose enum) must stay in sync
- Changes to platform role enum require:
  1. Update `PLATFORM_USER_ROLE_ENUM` in `server/models/index.ts`
  2. Update `lib/platformRoles.ts` to match
  3. Run `npm run migrate:platform-roles` if normalizing existing data
...
- Migrate legacy audit logs with `npm run migrate:*` scripts if needed
...
- MongoDB (default `mongodb://localhost:27018/governance`)
...
- `legacyFirebaseId` preserves migration audit trail without exposing Mongo `_id`
...
1. Define Mongoose schema in `server/models/index.ts`
2. Include common fields (legacyFirebaseId, createdBy, ownerId, timestamps)
3. Add virtual `id` getter
4. Export model via `getOrCreateModel()`
5. Reference in `registerMongoApiRoutes()` so generic `/api/db/:resource` endpoints work

### Migrating Firestore Data
1. Export Firestore collection as JSON
2. Run `npm run migrate:import path/to/export.json`
3. Run `npm run migrate:validate` to verify relationships
4. Run targeted migrations as needed (e.g., `npm run migrate:platform-roles`)
